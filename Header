...
<div id="install">
<div>
 <a onclick="location.href='/Salody/Member_Join' " id="install1">샐로디 처음이신가요? 회원가입/ 앱 설치하고 5,000P 할인받으세요! (즉시 사용가능)
	</a>
</div>
</div>
...
<div style="width: 100%; ">
	...
	<c:choose>
		...
		<c:otherwise>
			<a class="nav" href="Member_Login">로그인</a>&nbsp;&nbsp;
		</c:otherwise>
	...
	<c:choose>
		<c:when test = "${loginId!=null}">
			<a class="nav" href="Member_Mypage?id=${loginId}">마이페이지</a>&nbsp;&nbsp;
		</c:when>
		<c:otherwise>
	...
	<a class="nav" href="Board_List">공지사항 | 이벤트</a>&nbsp;&nbsp;
